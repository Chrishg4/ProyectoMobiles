...
EditText edSearch;
    ...
    public void EliminarPorNombre(String nombre) {
        ...
        if (!nombre.isEmpty()) {
            int registrosEliminados = BaseDatos.delete("entrenadores", "nombre=?", new String[]{nombre});
            BaseDatos.close();

            if (registrosEliminados > 0) {
                Toast.makeText(getApplicationContext(), "Registros eliminados correctamente", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(getApplicationContext(), "No se encontraron registros con ese nombre", Toast.LENGTH_LONG).show();
            }
        } else {
    ...
    public void Editar(View view) {
        if (itemseleccionado >= 0) {
            Intent intent = new Intent(this, Activity_AdmEntrenadores.class);
            intent.putExtra("entrenador", adapter.getItem(itemseleccionado));
            startActivity(intent);
        } else {
            Toast.makeText(getApplicationContext(), "Debe seleccionar un item", Toast.LENGTH_SHORT).show();
        }
    ...
    private void cargarEntrenadores() {
        ...
        Cursor cursor = db.rawQuery("SELECT nombre, contacto FROM entrenadores", null);
